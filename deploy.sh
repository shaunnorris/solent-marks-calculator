...
server {
    ...
    # Analytics tracking script (must come before /api/)
    location /script.js {
    ...
    # Analytics dashboard and auth
    location ~ ^/(login|dashboard|settings|reports|realtime|profile|teams|websites) {
    ...
    location /_next/ {
        proxy_pass http://umami:3000/_next/;
        proxy_set_header Host \$host;
        add_header Cache-Control "public, max-age=3600";
    }
